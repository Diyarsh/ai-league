...
const models = [
  { name: 'DeepSeek V3.1', color: '#4C9AFF', volatility: 0.015, icon: DeepSeek },
  { name: 'Claude 4.5', color: '#9D7BE8', volatility: 0.012, icon: Claude },
  { name: 'GPT 5', color: '#FF6B6B', volatility: 0.018, icon: OpenAI },
  { name: 'Gemini 2.5 Pro', color: '#FFA500', volatility: 0.010, icon: Gemini },
  { name: 'Grok 4', color: '#000000', volatility: 0.020, icon: XAI },
];
...
const CustomDot = (props: any) => {
  const { cx, cy, payload, dataKey, index, data } = props;
  ...
  return (
    <g>
      <foreignObject x={cx - 16} y={cy - 16} width={32} height={32}>
        <div className="w-8 h-8 rounded-full bg-card border-2 flex items-center justify-center shadow-lg" style={{ borderColor: model.color }}>
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
              timeRange === 'all' ? 'bg-foreground text-background' : 'bg-secondary text-foreground hover:bg-secondary/80'
            }`}
          >
            ALL
          </button>
          <button 
            ...
            className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
              timeRange === '72h' ? 'bg-foreground text-background' : 'bg-secondary text-foreground hover:bg-secondary/80'
            }`}
          >
            72H
          </button>
          <button 
            ...
            className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
              isLive ? 'bg-success text-white' : 'bg-secondary text-foreground hover:bg-secondary/80'
            }`}
          >
